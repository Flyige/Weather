...
public class NoteDbOpenHelper extends SQLiteOpenHelper {
    ...
    public long insertData(NoteBean note) {
        SQLiteDatabase db= getWritableDatabase();
        //ContentValues用键值对方式存储
...
//        return db.delete(TABLE_NAME_NOTE, "id is ?", new String[]{id});
    ...
    public int updateData(NoteBean note) {

        SQLiteDatabase db = getWritableDatabase();
        Log.d(TAG, "updateData:执行保存了 ");
        ContentValues values = new ContentValues();
        values.put("title", note.getTitle());
        values.put("content", note.getContent());
        values.put("create_time", note.getCreatedTime());
    ...
    public List<NoteBean> queryAllFromDb() {
    ...
    public List<NoteBean> queryFromDbByTitle(String title) {
